    ...
    public class GameplayAbilitySpec
    {
        ...
        public bool IsOnCooldown()
        {
            ...
            if (cdEffect != null)
            {
                ...
                if (cdEffect.IsTurnBasedCooldown)
                    return cdEffect.RemainingCooldownTurns > 0;
            }
        ...
        public SkillCooldownInfo GetCooldownInfo()
        {
            ...
            info.IsTurnBasedCooldown = cdEffect.IsTurnBasedCooldown;

            if (cdEffect.IsTurnBasedCooldown)
            {
                info.RemainingTurns = cdEffect.RemainingCooldownTurns;
                info.TotalTurns = cdEffect.TotalCooldownTurns;
                info.IsOnCooldown = cdEffect.RemainingCooldownTurns > 0;
            }
            else if (cdEffect.IsChargeCooldown)
            {
        ...
        private void PlayAnimation(string name, bool loop)
        {
            ...
            var player = GASBridge.AnimationPlayerProvider?.Invoke(Owner.Owner);
            player?.PlayAnimation(name, loop);
        }
